...
def measure_deep_size(obj: Any) -> int:
    ...
    def _get_size(o: Any) -> int:
        ...
        if isinstance(o, dict):
            ...
            size += sum(_get_size(getattr(o, slot)) for slot in o.__slots__ if hasattr(o, slot))
...
def print_result(name: str, value: float, unit: str = 'ms', name_width: int = 40) -> None:
...
def require_import(module_name: str) -> Any:
    ...
    if module is None:
        raise ImportError(f"Required module '{module_name}' is not installed. Run: uv pip install {module_name}")
    return module
