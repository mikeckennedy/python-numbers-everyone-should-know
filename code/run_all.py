...
from utils.benchmark import BenchmarkResult, MemoryResult
    ...
    for category_key, category_data in all_results["categories"].items():
        for result in category_data["results"]:
            ...
            if any(kw in name.lower() for kw in [
                "empty function", "dict lookup", "list append",
                "json.dumps", "isinstance", "try/except"
            ]):
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Python Numbers Everyone Should Know benchmarks"
    )
    parser.add_argument(
        "--quick",
        action="store_true",
    ...
    if args.category:
        ...
        categories_to_run = {
            k: v for k, v in BENCHMARK_CATEGORIES.items()
            if k in QUICK_CATEGORIES
        }
