...
def run_benchmarks() -> list[BenchmarkResult]:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        def json_extract_simple():
            cur = conn.execute(
                "SELECT json_extract(data, '$.username') FROM users WHERE id = ?",
                (test_id,)
            )
            return cur.fetchone()
        ...
        def json_extract_nested():
            cur = conn.execute(
                "SELECT json_extract(data, '$.profile.location') FROM users WHERE id = ?",
                (test_id,)
            )
            return cur.fetchone()
        ...
        def json_extract_array():
            cur = conn.execute(
                "SELECT json_extract(data, '$.posts[0].title') FROM users WHERE id = ?",
                (test_id,)
            )
            return cur.fetchone()
